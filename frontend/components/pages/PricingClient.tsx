    ...
    <div className="min-h-screen bg-[#090909] text-white overflow-x-hidden">
      ...
      <section className="relative pt-40 pb-16 overflow-hidden">
        <div className="container max-w-[1400px] relative z-10 px-4 sm:px-6">
          <motion.div className="inline-flex items-center gap-2 px-4 py-1.5 bg-white/[0.04] border border-white/[0.08] rounded-full text-sm text-[#6EE7B7] mb-6" initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <Tag className="w-3.5 h-3.5" /><span className="font-mono uppercase tracking-wider text-xs">Pricing</span>
          </motion.div>
          <motion.h1 className="text-4xl sm:text-6xl font-bold mb-5 leading-[1.1] tracking-tight" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.1, ease }}>
            Straightforward Pricing
          </motion.h1>
          <motion.p className="text-lg text-white/60 max-w-xl leading-[1.8]" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.2, ease }}>
            What each engagement costs to start, and what&apos;s included. Talk to us if your project needs something in between.
          </motion.p>
        </div>
      </section>
          ...
          {loading ? (
            ...
            <div className="border-t border-white/[0.08]">
              {tiers.map((tier, i) => (
                ...
                >
                  <Link href="/contact" className="grid lg:grid-cols-[3rem_1fr_auto] gap-4 lg:gap-10 items-start lg:items-center py-10 sm:py-12">
