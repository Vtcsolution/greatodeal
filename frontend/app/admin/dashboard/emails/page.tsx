...
function EmailsPageInner() {
  ...
  const [filter, setFilter] = useState<'all' | 'new' | 'replied' | 'imported'>('all');
  ...
  const filtered = contacts.filter(c => {
    const matchesFilter = filter === 'all' || (filter === 'imported' ? c.source === 'lead_finder' : c.status === filter);
  ...
  const importedCount = contacts.filter(c => c.source === 'lead_finder').length;
    ...
    <div className="p-4 sm:p-6 lg:p-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6 sm:mb-8">
        <div>
          ...
          <p className="text-white/50 text-sm">{contacts.length} total &middot; <span className="text-emerald-400">{newCount} new</span> &middot; <span className="text-blue-400">{importedCount} imported</span></p>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        ...
        <div className="flex gap-2">
          {(['all', 'new', 'replied', 'imported'] as const).map(f => (
                  ...
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                        ...
                        {c.source === 'lead_finder' && (
                          <span className="text-[10px] px-1.5 py-0.5 rounded-full font-medium bg-blue-500/10 text-blue-400">Imported</span>
                        )}
                      </div>
